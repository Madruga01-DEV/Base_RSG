const InventoryContainer = Vue.createApp({
    ...
    computed: {
        ...
        playerPanelStyle() {
            if (this.playerPanelPos.x !== null && this.playerPanelPos.y !== null) {
                return `position: fixed; left: ${this.playerPanelPos.x}px !important; top: ${this.playerPanelPos.y}px !important; right: auto !important; transform: none !important;`;
            }
            return '';
        },
        otherPanelStyle() {
            if (this.otherPanelPos.x !== null && this.otherPanelPos.y !== null) {
                return `position: fixed; left: ${this.otherPanelPos.x}px !important; top: ${this.otherPanelPos.y}px !important; right: auto !important; transform: none !important;`;
            }
            return '';
        },
    },
    ...
    methods: {
        getInitialState() {
            return {
                ...
                // -------- Panel Drag System --------
                playerPanelPos: { x: null, y: null },
                otherPanelPos: { x: null, y: null },
                isDraggingPanel: false,
                draggingPanelType: null,
                panelDragStartX: 0,
                panelDragStartY: 0,
                panelDragOffsetX: 0,
                panelDragOffsetY: 0,
        ...
        // -------- Panel Drag Methods --------
        startPanelDrag(event, panelType) {
            event.preventDefault();
            event.stopPropagation();
            
            this.isDraggingPanel = true;
            this.draggingPanelType = panelType;
            
            const panel = event.target.closest('.inventory-panel');
            if (!panel) return;
        ...
        handleGlobalMouseMove(event) {
            // Panel dragging
            if (this.isDraggingPanel && this.draggingPanelType) {
                const newX = event.clientX - this.panelDragOffsetX;
                const newY = event.clientY - this.panelDragOffsetY;
                
                if (this.draggingPanelType === 'player') {
                    this.playerPanelPos = { x: newX, y: newY };
                } else if (this.draggingPanelType === 'other') {
                    this.otherPanelPos = { x: newX, y: newY };
                }
                return;
            }
            
            // Item dragging (original logic)
            this.drag(event);
        },
        ...
        getPlayerPanelStyle() {
            if (this.playerPanelPos.x !== null && this.playerPanelPos.y !== null) {
                return {
                    position: 'fixed',
                    left: this.playerPanelPos.x + 'px',
                    top: this.playerPanelPos.y + 'px',
                    right: 'auto',
                    transform: 'none'
                };
            }
        ...
        getOtherPanelStyle() {
            if (this.otherPanelPos.x !== null && this.otherPanelPos.y !== null) {
                return {
                    position: 'fixed',
                    left: this.otherPanelPos.x + 'px',
                    top: this.otherPanelPos.y + 'px',
                    right: 'auto',
                    transform: 'none'
                };
            }
        ...
        resetPanelPositions() {
            this.playerPanelPos = { x: null, y: null };
            this.otherPanelPos = { x: null, y: null };
        },
        // ------------------------------------
